...
public class DishesService {
    ...
    public Optional<Dish> findById(long id) {
        LOGGER.debug("findById() started with dishId=%d", id);
        Optional<Dish> foundDishOptional = dishesRepository.findById(id);
        LOGGER.info("findById() dish with id={}" + (foundDishOptional.isPresent() ? " found": " not found"), id);
        return foundDishOptional;
    }

    public List<Dish> findByNameStartingWith(String name) {
        LOGGER.debug("findByNameStartingWith() started with name={}", name);
        List<Dish> dishesList = dishesRepository.findByNameStartingWith(name);
        LOGGER.info("findByNameStartingWith() dishes" + (dishesList.isEmpty() ? " not found": " found"));
        return dishesList;
    }

    @Transactional
    public long save(DishDTO dish) throws DataIntegrityViolationException {
        LOGGER.debug("save() started with dishDTO={{}}", dish);
        try {
    ...
    public Dish update(long id, String name, int type, String description, boolean isActual) {
        LOGGER.debug("update() started with id={}, name={}, type={}, description=\"{}\", isActual={}", id, name, type, description, isActual);
        ...
        if (dishOpt.isPresent()) {
            ...
            try {
                ...
                LOGGER.info("update() data are saved in data storage ");
                return savedDish;
            }
            catch (DataIntegrityViolationException e) {
                LOGGER.error("update() data are not saved in data storage: {}", e.getMessage());
               throw new DataIntegrityViolationException(
                        String.format("The dish with the name=\"%s\" already exists", name)
                ); 
            }
        } else {
            LOGGER.error("update() data are not saved in data storage: Dish item with id={} not found",id);
    ...
    public Dish patch(long id, Optional<String> name, Optional<Integer> type, Optional<String> description, Optional<Boolean> isActual) {
        LOGGER.debug("patch() started with id={}, name={}, type={}, description=\"{}\", isActual={}", 
        ...
        if (dishOpt.isPresent()) {
            ...
            try {
                ...
                LOGGER.error("patch() data are not saved in data storage: %s", e.getMessage());
               throw new DataIntegrityViolationException(
                        String.format("The dish with the name=\"%s\" already exists", name.get())
                ); 
            }
        } else {
            LOGGER.error("patch() data are not saved in data storage: Dish item with id={} not found",id);
            throw new EntityNotFoundException(String.format("Dish item with id=%d does not exist", id));
        }
    }
